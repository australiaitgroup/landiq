...
const stages = [
  {
    ...
    activities: [
      ...
      "Stand up the multi-agent harness (planner · specialists · critic · composer + ROIAgent) with step/cost budgets",
      "Implement ROI / feasibility calculation: IRR, gross development yield, development margin from FSR × indicative sale price × indicative build cost, with assumption registry and inline assumption printing",
      "Calibrate model against the 5 Calibration Councils: tune templates, validate prompts against ground-truth deals, handle council-specific edge cases",
    ],
    deliverables: [
      "End-to-end report generator with PDF/DOCX export",
      "ROI / feasibility section in every generated report — IRR, gross yield, development margin, with explicit assumptions",
    ...
    acceptance: [
      "Report quality ≥ 85% against Client benchmark rubric on the 5 Calibration Councils",
      "ROI section reconciles within ±10% of Mountain Property's manual workbook on benchmark sites (where workbook is shared)",
      "Average generation end-to-end ≤ 60 seconds",
      "Outside the 5 Calibration Councils: best-efforts standard template (no per-council quality guarantee)",
    ],
  },
  {
    ...
    activities: [
      ...
      "Build Excel feasibility workbook integration — import Client's workbook structure into the platform's ROI engine; export the AI-generated ROI section back to a matching Excel layout per report",
    ...
    deliverables: [
      ...
      "Excel workbook integration — bidirectional import (workbook structure → platform inputs) and per-report export (platform ROI section → Excel layout matching Client's working template)",
    ...
    acceptance: [
      ...
      "Excel workbook round-trip verified on one Mountain Property reference site (import workbook → generate report → export ROI → values reconcile to the source workbook within ±10%)",
